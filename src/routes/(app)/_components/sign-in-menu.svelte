...
{:else}
	<div class="signedInText" on:click|stopPropagation={toggleSignInMenu} on:keydown={handleKeypress}>
		<img class="user-signed-in-icon" alt="User signed in icon" src="/images/icons/user-icon.svg" />
		<p>{$page.data.user.firstname} {$page.data.user.lastname}</p>
		<img class="expand-user-menu" alt="expand-user-options" src="/images/icons/arrow-down.png" />
	</div>
	{#if signInMenuOpen}
		<div
